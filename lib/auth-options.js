...
async function findOrCreateProfile(user) {
  const existing = await pool.query('SELECT id, user_id, full_name, email, credits_balance FROM public.profiles WHERE user_id = $1 LIMIT 1', [user.id])
  if (existing.rows[0]) return existing.rows[0]
  const neonProfile = await pool.query('SELECT id FROM neon_auth.user WHERE lower(email) = lower($1) LIMIT 1', [user.email])
  if (!neonProfile.rows[0]) return null
  const created = await pool.query(`INSERT INTO public.profiles (id, user_id, email, full_name) VALUES ($1, $2, $3, $4) ON CONFLICT (id) DO UPDATE SET user_id = EXCLUDED.user_id, email = EXCLUDED.email, full_name = EXCLUDED.full_name RETURNING id, user_id, full_name, email, credits_balance`, [neonProfile.rows[0].id, user.id, user.email, user.name || null])
  return created.rows[0] || null
}

async function upsertUser({ email, name, googleId }) {
  const result = await pool.query(`INSERT INTO public.users (email, name, google_id) VALUES ($1, $2, $3) ON CONFLICT (email) DO UPDATE SET name = COALESCE(EXCLUDED.name, public.users.name), google_id = COALESCE(EXCLUDED.google_id, public.users.google_id) RETURNING id, email, name`, [email, name || null, googleId || null])
